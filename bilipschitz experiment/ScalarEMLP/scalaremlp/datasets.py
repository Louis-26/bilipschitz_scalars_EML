...
class O5Synthetic(Dataset):
    def __init__(self, N=1024):
        ...
        self.Y = (r1 ** 2).sum(-1).sqrt().sin() - .5 * (r2 ** 2).sum(-1).sqrt() ** 3 + (r1 * r2).sum(-1) / (
                    (r1 ** 2).sum(-1).sqrt() * (r2 ** 2).sum(-1).sqrt())
...
class ParticleInteraction(Dataset):
    ...
    def __init__(self, N=1024):
        ...
        # does not violate equivariance
        self.Xscale = np.sqrt(
            (np.abs((self.X.reshape(N, 4, 4) @ 𝜂.numpy()) * self.X.reshape(N, 4, 4)).mean(-1)).mean(0))
...
class BrokenRubiksCube(Dataset):
    ...
    def __init__(self, train=True):
        ...
        self.X[11:] = solved_state.reshape(6, 6, 8)[:, parity_perm].reshape(
            -1)  # equivalence_classes.reshape(12,-1)[1:]
        self.Y = labels
