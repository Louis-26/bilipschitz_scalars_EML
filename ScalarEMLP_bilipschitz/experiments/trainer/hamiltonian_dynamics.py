...
# cap on solver steps per output interval: a badly initialised model (e.g. at step 0) can make the adaptive
# solver shrink its step forever; well-trained models need a few tens of steps per interval
MXSTEP = 2000
...
def BHamiltonianFlow(H,z0,T,tol=1e-4):
    ...
    # print(dynamics)
    return odeint(dynamics, z0, T, rtol=tol, mxstep=MXSTEP).transpose((1,0,2))
...
class HamiltonianDataset(Dataset):
    ...
    def __init__(self,n_systems=100,chunk_len=5,dt=0.2,integration_time=30,regen=False):
        ...
        # cache next to the package instead of the current working directory, overridable by $SCALAREMLP_DATA_DIR
        default_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "datasets")
        root_dir = os.path.join(os.environ.get("SCALAREMLP_DATA_DIR", default_root), "ODEDynamics", self.__class__.__name__)
...
class DoubleSpringPendulum(HamiltonianDataset):
    ...
    def H(self,z):
        ...
        # keep the upstream Hamiltonian (Finzi et al. 2021 / Yao et al. 2021) so that results are comparable with the paper
